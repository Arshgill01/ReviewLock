...
const markDedupe = async (
  ...
  if (await redis.exists(dedupe)) {
    return false;
  }

  await redis.set(dedupe, now);
  return true;
};
...
export const handleReportTrigger = async (
  ...
  if (!resolution.ok || !resolution.target) {
    await appendAuditEvent(deps.redis, {
      ...
      actor: 'reviewlock',
      createdAt: now,
      message: 'Report trigger could not resolve current content, so reports were not suppressed.',
  ...
  await appendAuditEvent(deps.redis, {
    ...
    actor: 'reviewlock',
    createdAt: now,
    message: 'Report trigger could not verify content integrity, so reports were not suppressed.',
